...
export async function update(ctx){
    try{
        ...
        // Check if this user can update this list.
        const listFound = await List.findById(ctx.params.id);

        if(!listFound || listFound.length=== 0) { return ctx.notFound() }
        
        if(!List.belongsToUserConnected(listFound, ctx.state.user.id)){
            return ctx.unauthorized({message:"This user cannot update this list."})
        }
...
export async function deleteById(ctx){
    try{
        ...
        // Check if this user can delete this list.
        const listFound = await List.findById(ctx.params.id);
