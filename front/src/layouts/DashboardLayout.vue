    ...
    <q-layout view="hhh lpR lFf">
        ...
        <q-drawer show-if-above v-model="leftDrawerOpen" side="left" class="q-pa-lg bg-grey flex">
            <div class="flex column justify-between">
                <div>
                    <div class="flex justify-between items-center row q-mb-lg">
                        <h4>Mes listes</h4>
                        <q-btn label="+" outline/>
                    </div>
                    <div v-if="listsFound.length!==0" class="flex column q-gutter-lg">
                        <div v-for="(list, index) in listsFound" :key="index">
                            <h5 class="text-weight-normal cursor-pointer" @click="listStore.setSelectedList(list._id); router.push({name:'list', params:{id:list._id}})"> {{list.title}}</h5>
                        </div>
                    </div>
                    <div v-else>
